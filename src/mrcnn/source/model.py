...
class MaskRCNN():
    ...
    def detect(self, images, verbose=0):
        ...
        # Build for Timeline/Profiling purpose
        run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
        run_metadata = tf.RunMetadata()
        
        optimizer = keras.optimizers.SGD(lr=0.001, momentum=self.config.LEARNING_MOMENTUM,
                                         clipnorm=5.0)
        self.keras_model.compile(optimizer=optimizer, loss=[
                                 None] * len(self.keras_model.outputs), options=run_options, run_metadata=run_metadata)
